...
Page({
  ...
  formatDate(date) {
    ...
    let searchWeek = date.getDay()
    if (searchWeek == 0) {
      searchWeek = 7
    }
    let searchWhichWeek = dateUtil.getWhichWeekByDate(date)

    // console.log("周：" + searchWeek)
    // console.log("第几周：" + dateUtil.getWhichWeekByDate(date))
  ...
  searchDetail: function (e) {
    ...
    let params = {
      ...
      "whichweek": this.data.searchWhichWeek,
      "week": this.data.searchWeek
    }
    ...
    if (date == "今天") {
      ...
      roomService.getDetailRoomStatus(params, this.handleResSuccess);
    }
  ...
  onLoad: function () {
    ...
    let capsuleBtnInfo = app.globalData.capsuleBtnInfo //wx.getMenuButtonBoundingClientRect()
    this.setData({
      wetherTop: capsuleBtnInfo.top,
      wetherHeight: capsuleBtnInfo.height
    })

    // 检查有没有token， 没有的话，跳转到empty
    let token = app.globalData.token
    // 判断是否是从empty来的
    console.log(app.globalData.fromEmpty)
    if (!app.globalData.fromEmpty || app.globalData.fromEmpty == null || app.globalData.fromEmpty == "") {
      // 判断本地是否有token
      wx.getStorage({
