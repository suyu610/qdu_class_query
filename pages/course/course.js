...
Page({
  ...
  data: {
    colorPattern:[
      {color0:"#FF5252",color1:"#FFE57F",color2:"#1565C0"},
      {color0:"#FCE4EC",color1:"#E57373",color2:"#FFE0B2"}
    ],
    today:today,
    turn_img:"/images/room_dir_0.png",
    holiday:{"name":"劳动节","countdown":"2021/5/1 00:00:00"},    
    countdown:{"day":0,"hour":0,"min":0,"sec":0},
    holidays:{},
    exams:{},
    friend_realname:"",
    bindFriendToken:"",
  ...
  toggleShowFriendCourse:function(e){
    wx.setStorage({key:"showFriendCourse",data:e.detail})
    this.setData({showFriendCourse:e.detail})
  },
  ...
  onTapFolderMenu:function(e){
    wx.setStorage({key:"folderMenu",data:!this.data.folderMenu})
    this.setData({folderMenu:!this.data.folderMenu})
  },
  onSwtichShowFriendsCourse:function(e){
    this.setData({showFriendsCourse:e.detail})
  },
  ...
  onRefresh:function(){
    this.setData({activeNames:['1'],loading:false})
  ...
  handleCheckAlarmSuccess:function(e){
    ...
    if(e == 1){
      ...
      wx.hideLoading({
        success: (res) => {
          this.setData({openAlarmValue:true})
          wx.showToast({
  ...
  // 如果没有订阅，则弹窗
    ...
    wx.hideLoading({
      success: (res) => {
        this.setData({openAlarmValue:false})
        this.setData({showSettingPopup:false})
        this.setData({modalAlarmFailureShow:true})        
      },
    })
  }
  },
  ...
  onClickClock:function(e){
    this.setData({showClock:true,showSettingPopup:false})
  },

  clockClose:function(e){
    this.setData({showClock:false,showSettingPopup:true})
  ...
  importBtnHandler:function(){
    this.setData({ showImportPopup: true });
  },
  // 关闭导入课表页面
  ...
  selectBg:function(e){
    ...
    if(this.data.selectBgIndex == e.currentTarget.dataset.index){
      this.setData({selectBgIndex:-1,backgroundUrl:""})    
      return  
    }
    // 如果选择的是自己的背景
    if(e.currentTarget.dataset.index == this.data.bgFileList.length){
      this.setData({selectBgIndex,backgroundUrl:this.data.fileList[0].url})
      return
    }  
    this.setData({selectBgIndex,backgroundUrl:this.data.bgFileList[selectBgIndex]})    
  },
  ...
  uploadToCloud(event) {
    ...
    if (!fileList) {
      ...
      let localBgImg = fileList[0].file.url;
        that.setData({ 
          fileList:[{url:localBgImg,"isImage":true}]         
        });      
  ...
  showCardView: function (event) {
    ...
    if (course.direction == 0){
      ...
      if(course.rFloor==1){
        turn_img = '/images/room_dir_1_'+course.direction+'.png';
      }else{
        turn_img = '/images/room_dir_2_'+course.direction+'.png';
      }
    }
  ...
  judgeType:function(friendCourse,courselist,index){
    ...
    let  course = {
      ...
      course:friendCourse.course_name,
      teachers:friendCourse.teachers,
      place: friendCourse.class_name,
      week:friendCourse.week,
      info_str:friendCourse.info_str,
    ...
    courselist.forEach((e) => {      
      if(e.week == friendCourse.week && e.start == friendStart){
        ...
        course = {
          ...
          myCourse:e.course,
          friendCourse:friendCourse.course_name,
          myTeachers:e.teachers,
          friendTeachers:friendCourse.teachers,
          myPlace: e.place,
          friendPlace: friendCourse.class_name,
          week:e.week,
          myInfo_str:e.info_str,
          friendInfo_str:friendCourse.info_str,
          rFloor:e.rfloor,
...
deleteBgImg:function(e){
  ...
  getWholeBackgroundImage:function(){
    ...
    if(localBgImg=="" || localBgImg==null) return     
      this.setData({localBgImg:localBgImg.data,fileList:[{url:"data:image/png;base64,"+localBgImg.data,"isImage":true}]})    
    // 从云端获取
    // let that = this
  ...
  startCountdown(){
    ...
    this.setData({
      countdown:{"day":day,"hour":hour,"min":min,"sec":sec},
    })
  ...
  importByHand:function(){
    router.push({'name':'course_search'})
    // this.setData({
  ...
  importByDean:function(){
    router.push({name:'course_import'})
  },

  handleGetMyJwCourse:function(e){
    // 将它存到本地
    wx.setStorage({key:"my-jw-course-v1",data:e})
    this.setData({selfJwCourse:e})
    this.updateWeeks(today)
  },

  handleGetMyAddCourse:function(e){
    this.setData({selfAddCourse:e})
    this.updateAddCourse(today)
  },

  handleGetFriendJwCourse:function(e){
    var friend_realname = app.globalData.friend_realname;
    
    // 将它存到本地    
    wx.setStorage({key:friend_realname + "-jw-course",data:e})
    this.setData({friendJwCourse:e})    
    this.updateWeeks(today)
  },
  ...
  handleCheckBindFriend:function(e){
    this.setData({bindFriendToken:e})
  },
  ...
  initFriendCourse:function(){
    ...
    let friend_realname = app.globalData.friend_realname
    ...
    if(friend_realname == ""){
      ...
      if(friend_realname == '未导入教务课表') return
      //否则检查本地是否有该用户的课程数据
      wx.getStorage({
        key:friend_realname + "-jw-course",
        success (res) {
          if(res.data.length == 0){
            // 等同于本地没有值
            ...
            // 本地有值，且长度不为0
            that.setData({friendJwCourse:res.data})            
          }
        },
        fail(){
  ...
  handleGetCountdownAfterTodaySuccess:function(e){
    ...
    let holidays = [];
    let exams = [];
    let holiday = {"name":"Null","countdown":new Date("2099-01-01")};
    
    e.forEach(function(item){
      let tmpStartDate = new Date(item.start_date)
      let tmpItem = {
        "start_date":dateFormatUtil.formatTime2(tmpStartDate),
        "name":item.name,
        "gap_day":dateFormatUtil.gapDay(tmpStartDate,today),
        // 放几天假
        "duration":item.duration,
        "sign_date":dateFormatUtil.formatTime2(new Date(item.sign_date)),
      }
    ...
    this.setData({holiday,holidays,exams})
  },
  ...
  onLoad: function () {    
    ...
    if(app.globalData.url == "course" && app.globalData.params != null){
      ...
      if(selectBgIndex == this.data.bgFileList.length){
        ...
        this.setData({backgroundUrl:this.data.fileList[0].url})
        }
        return
      }
      ...
      this.setData({backgroundUrl:this.data.bgFileList[selectBgIndex]})
    }
  },
  ...
  setColor(type,color){    
    ...
    switch(type){
      case '0' :{
        ...
        this.setData({
          type0CourseColor:color,type0CourseTextColor:textColor
        })
        break;
      }
      case '1' :{
        ...
        this.setData({
          type1CourseColor:color,type1CourseTextColor:textColor
        })
        break;
      }
      case '2' :{
        ...
        this.setData({
          type2CourseColor:color,type2CourseTextColor:textColor
        })
        break;
      }
    }
